...
pos = ones(1,length(features_pos(:,1)));
neg = -ones(1,length(features_neg(:,1)));
Y = cat(2,pos,neg);
X = cat(2,features_pos',features_neg');
lambda = 0.0001;
maxIter = 1000;
...
[w, b] = vl_svmtrain(X, Y, lambda,'MaxNumIterations', maxIter);
...
imhog = vl_hog('render', single(reshape(w, [n_hog_cells n_hog_cells 31])), 'verbose','numOrientations', 9) ;
