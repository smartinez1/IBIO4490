    ...
    os.system('rm -rf productImages')
    os.system('mkdir productImages')
    ...
    plt.savefig('productImages/data_show')
    #plt.show()
    ...
    def imshow_label(label_show):
        ...
        plt.savefig('productImages/label')
        #plt.show()    
    ...
    class Trainer(object):
        ...
        def validate(self):
            ...
            plt.savefig('productImages/visualization')
            #plt.show()
            ...
            for batch_idx, (data, target) in tqdm.tqdm(
                ...
                with open(osp.join(self.out, 'log.csv'), 'a') as f:
                    elapsed_time = (
                        datetime.datetime.now(pytz.timezone('America/Bogota')) -
                        self.timestamp_start).total_seconds()
                    log = [self.epoch, self.iteration] + [loss.item()] +                     metrics.tolist() + [''] * 5 + [elapsed_time]
                    log = map(str, log)
                    f.write(','.join(log) + '\n')
